...
class Manager extends Admin_Controller
{
	...
	protected $_manager_validation_rules = array (
  ...
  public function index()
  {
    ...
    $str = '<a href="'.site_url('manager/add/{id}').'" class="btn btn btn-padding yellow table-action"><i class="fa fa-edit edit"></i></a><a href="javascript:void(0);" data-original-title="Remove" data-toggle="tooltip" data-placement="top" class="table-action btn-padding btn red" onclick="delete_record(\'manager/delete/{id}\',this);"><i class="fa fa-trash-o trash"></i></a>';
    
    $this->listing->initialize(array('listing_action' => $str));

    $listing = $this->listing->get_listings('manager_model', 'listing');
    $this->data['btn'] = "<a href=".site_url('manager/add')." class='btn green'>Add New Manager <i class='fa fa-plus'></i></a>";
    ...
    $this->data['grid'] = $this->load->view('listing/view', $this->data, TRUE);
  	$this->layout->view('/frontend/manager/index');
  }
  ...
  function add($edit_id =''){
       ...
       try
        {
            ...
            if ($this->form_validation->run())
            {
                ...
                if($edit_id)
                {
                  ...
                  $this->manager_model->update(array("id" => $edit_id),$ins_data);
                ...
                {    
                  ...
                  $this->manager_model->insert($ins_data);
        ...
        if($edit_id)
            $edit_data =$this->manager_model->get_where(array("id" => $edit_id))->row_array();

        $this->data['editdata']  = $edit_data;

        $this->layout->view('frontend/manager/add');
   }
    ...
    function check_email($mail,$id)
    {
      ...
      if($id)
          $where['id !='] = $id;
      $where['email'] = $mail;
      $result = $this->manager_model->get_where($where)->num_rows();     
      if ($result) {
    ...
    {
      $access_data = $this->manager_model->get_where(array("id"=>$del_id),'id')->row_array();     
      $output=array();
      if(count($access_data) > 0)
      {
        $this->manager_model->delete(array("id"=>$del_id));
